...
from herald.modules.email_generator.utils import load_filters_from_registry, get_environment


def test_load_filters_from_registry(
    mocker: MockerFixture, registry_json_data: str, mock_env: Environment
):
...
def test_load_filters_from_registry_no_registry_json(caplog: LogCaptureFixture, mock_env: Environment):
...
def test_load_filters_from_registry_invalid_registry_not_a_json(
    mocker: MockerFixture, caplog: LogCaptureFixture, mock_env: Environment
):
    mocker.patch.object(Path, "exists", return_value=True)
    mocker.patch(
        "herald.modules.email_generator.utils.open",
    ...
    assert (
        "Error loading filter registry from /test/filters_registry.json:"
...
def test_load_filters_from_registry_invalid_registry_json(
    ...
    assert (
        "Invalid filter registry format: /test/filters_registry.json "
        "should contain a list of filters."
    ) in caplog.text
...
def test_load_filters_from_registry_no_filter_name(
    ...
    assert (
        "Invalid filter entry in registry: {'entrypoint': 'myfilters.dummy_filter'}. Error: 'name'"
    ) in caplog.text
...
def test_load_filters_from_registry_no_filter_entrypoint(
    ...
    assert (
        "Invalid filter entry in registry: {'name': 'dummy'}. Error: 'entrypoint'"
    ) in caplog.text


def test_load_filters_from_registry_import_error(
    mocker: MockerFixture, caplog: LogCaptureFixture, registry_json_data: str, mock_env: Environment
):
    mocker.patch.object(Path, "exists", return_value=True)
    mocker.patch(
        "herald.modules.email_generator.utils.open",
    ...
    assert (
        "Failed to load filter 'dummy': import error"
    ) in caplog.text
...
def test_load_filters_from_registry_attribute_error(
    ...
    assert (
        "Failed to load filter 'dummy': module 'myfilters' has no attribute 'dummy_filter'"
    ) in caplog.text
