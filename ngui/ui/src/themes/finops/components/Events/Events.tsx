...
const EventList = ({
  ...
  const renderAccordion = (eventsList, parentIndex) =>
    eventsList.map((event, index) => (
      <Accordion
        ...
        hideExpandIcon
      >
        <Box maxWidth="100%" display="flex" alignItems="center" py={0.5} sx={{ paddingTop: 2, paddingBottom: 2 }}>
  ...
  const renderEventList = () => {
    ...
    if (noEvents) {
      return (
        <Box display="flex" justifyContent="left" sx={{ paddingBottom: 2, paddingTop: 2 }}>
    ...
    return (
      <Box display="flex" flexDirection="column" flexGrow={1} flexBasis="auto">
        <Box sx={{ marginTop: MPT_SPACING_2 }}>
