...
import Header from "@main/components/DateRangePicker/Header";
import useStyles from "@main/components/DateRangePicker/Month/Month.styles";
          ...
          {getChunks(getDaysInMonth(date), 7).map((week) => (
            <Grid key={JSON.stringify(`${week[0]}`)} container direction="row" justifyContent="center">
              {week.map((day) => {
                ...
                const outsideMonth = !isSameMonth(date, day);
                const outsideBounds = !isWithinInterval(day, { start: minDate, end: maxDate });
                return (
                  <Day
                    ...
                    disabled={outsideMonth || outsideBounds}
                    outsideMonth={outsideMonth}
                    outsideBounds={outsideBounds && !outsideMonth}
