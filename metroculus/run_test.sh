...
do
    ...
    docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
        "uv --project metroculus run pycodestyle --exclude=.venv --max-line-length=120 metroculus"
    echo "<<<Pycodestyle tests"

    echo "Pylint tests>>>"
    docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
        "uv --project metroculus run pylint --rcfile=metroculus/.pylintrc --fail-under=9 --fail-on=E,F ./metroculus/${SERVICE}"
    echo "<<<Pylint tests"

    if [[ "${SERVICE}" == "metroculus_api" ]]; then
        echo "Unit tests>>>"
        docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
            "uv --project metroculus run python -m unittest discover ./metroculus/${SERVICE}/tests"
        echo "<<Unit tests"
    fi
