...
do
    ...
    docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
        "uv --project insider run pycodestyle --exclude=.venv --max-line-length=120 insider"
    echo "<<<Pycodestyle tests"

    echo "Pylint tests>>>"
    docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
        "uv --project insider run pylint --rcfile=insider/.pylintrc --fail-under=9 --fail-on=E,F ./insider/${SERVICE}"
    echo "<<<Pylint tests"

    if [[ "${SERVICE}" == "insider_api" ]]; then
        echo "Unit tests>>>"
        docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG}  bash -c \
            "uv --project insider run python -m unittest discover ./insider/insider_api/tests"
        echo "<<Unit tests"
    fi
