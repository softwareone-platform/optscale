...
do
    ...
    docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
        "uv --project risp run pycodestyle --exclude=.venv --max-line-length=120 risp"
    echo "<<<Pycodestyle tests"

    echo "Pylint tests>>>"
    docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} bash -c \
        "uv --project risp run pylint --rcfile=risp/.pylintrc --fail-under=8 --fail-on=E,F ./risp/${SERVICE}"
    echo "<<<Pylint tests"

    if [[ "${SERVICE}" == "risp_worker" ]]; then
        echo "Worker tests>>>"
        docker run -i --rm ${TEST_IMAGE}:${BUILD_TAG} \
            bash -c "uv --project risp run python risp/run_tests.py"
        echo "<<Worker tests"
    fi
