...
test.describe('MPT-8230 Invitation Flow Tests for new users', { tag: ['@invitation-flow', '@ui', '@slow'] }, () => {
  ...
  test(
    '[229866] Invite new user to organisation, but user declines',
    { tag: '@p2' },
    async ({ header,
             mainMenu,
             usersPage,
  ...
  test(
    '[229867] Invite new user to organisation, who has previously declined @slow',
    { tag: '@p2' },
    async ({
      header,
      loginPage,
...
test.describe('MPT-8229 Validate invitations in the settings', { tag: ['@invitation-flow', '@ui', '@slow'] }, () => {
  ...
  test(
    '[229868] Invitation is visible in Settings Tab @slow',
    { tag: '@p2' },
    async ({
      loginPage,
      header,
...
test.describe('MPT-8231 Invitation Flow Tests for an existing user', { tag: ['@invitation-flow', '@ui', '@slow'] }, () => {
  ...
  test(
    '[229869] Invite existing user with a new role @slow',
    { tag: '@p2' },
    async ({
      header,
      loginPage,
...
test.describe(
  ...
  () => {
    ...
    test(
      '[232868] Invite a new user and verify the event is logged',
      { tag: '@p2' },
      async ({ mainMenu, usersPage, usersInvitePage, eventsPage }) => {
        invitationEmail = generateRandomEmail();
        let date: string;
