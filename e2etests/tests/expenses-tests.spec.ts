...
test.describe('[MPT-12859] Expenses Page default view Tests', { tag: ['@ui', '@expenses'] }, () => {
  ...
  test(
    '[231212] Breakdown by Geography button navigates to Cost map page',
    { tag: ['@fast', '@p2'] },
    async ({ expensesPage, expensesMapPage }) => {
...
test.describe('[MPT-12859] Expenses Page Source Breakdown Tests', { tag: ['@ui', '@expenses'] }, () => {
  ...
  test(
    '[231217] Verify data source expenses total for (last month) period matches chart and table totals',
    { tag: ['@fast', '@p2'] },
    async ({ expensesPage, datePicker }) => {
...
test.describe('[MPT-12859] Expenses Page Pool Breakdown Tests', { tag: ['@ui', '@expenses'] }, () => {
  ...
  test(
    '[231220] Verify pool expenses total for(default) period matches chart and table totals',
    { tag: ['@fast', '@p2'] },
    async ({ expensesPage }) => {
      const totalForPeriod = await expensesPage.getTotalExpensesForSelectedPeriod();
      debugLog(`Total expenses for selected period: ${totalForPeriod}`);
  ...
  test(
    '[231221] Verify pool expenses total for (last 7 days) period matches chart and table totals',
    { tag: ['@fast', '@p2'] },
    async ({ expensesPage, datePicker }) => {
      await datePicker.selectLast7DaysDateRange();
...
test.describe('[MPT-12859] Expenses Page Owner Breakdown Tests', { tag: ['@ui', '@expenses'] }, () => {
  ...
  test(
    '[231224] Verify owner expenses total for(default) period matches chart and table totals',
    { tag: ['@fast', '@p2'] },
    async ({ expensesPage }) => {
      const totalForPeriod = await expensesPage.getTotalExpensesForSelectedPeriod();
      debugLog(`Total expenses for selected period: ${totalForPeriod}`);
  ...
  test(
    '[231225] Verify owner expenses total for (last 30 days) period matches chart and table totals',
    { tag: ['@fast', '@p2'] },
    async ({ expensesPage, datePicker }) => {
