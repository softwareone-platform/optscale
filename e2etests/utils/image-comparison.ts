 ...
 */
export async function comparePngImages(imgPath1: string, imgPath2: string, diffPath?: string): Promise<boolean> {
  ...
  const numDiffPixels = pixelmatch(paddedImg1.data, paddedImg2.data, diff.data, width, height, { threshold: 0, diffMask: false });
