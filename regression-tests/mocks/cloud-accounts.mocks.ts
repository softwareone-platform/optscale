...
import { AZURE_CLOUD_ACCOUNT_ID, ORGANIZATION_ID_CLOUD_ACCOUNTS, SETTINGS_EMPLOYEE_ID } from './ids';
  ...
  data: {
    dataSources: [
      ...
      },
      {
        ...
        last_getting_metric_attempt_error:
          "{'code': 'ServerTimeout', 'message': \"The request timed out. Diagnostic information: timestamp '20251130T000714Z', subscription id '', tracking id 'fe0abbfa-9b6f-4eaf-8407-14d1ca67a330', request correlation id 'fe0abbfa-9b6f-4eaf-8407-14d1ca67a330'.\"}",
...
const PoolMock = {
  ...
  limit: 15000,
  name: SAMPLE_NAME,
  organization_id: ORGANIZATION_ID_CLOUD_ACCOUNTS,
  parent_id: null,
...
/** Data sources carrying an `entitlement` tag, keyed by name — drives the "Entitled" column. */
const ENTITLEMENT_TAGS: Record<string, string> = {
  'Marketplace (Dev)': 'FENT-6650-1643-9450',
};

const TENANT_NAME = e2e('SoftwareOne');

/**
 * `GET /v1/client/organizations/<id>/datasources` — the FFC REST list behind the "Entitled"
 * column. `DataSourceTagCell` looks a row up by its data-source id, so this is derived from
 * `DataSourcesMock` rather than copied: hardcoded ids would silently stop matching the table
 * and every cell would render "No". A row absent from `ENTITLEMENT_TAGS` renders "No".
 */
const ClientDataSourcesMock = DataSourcesMock.data.dataSources.map(({ id, name, type, account_id, parent_id, details }) => ({
  id,
  name,
  type,
  account_id,
  parent: parent_id ? { id: parent_id, name: TENANT_NAME, type: 'azure_tenant' } : null,
  tags: ENTITLEMENT_TAGS[name] ? [{ id: 'de7624f4-b915-42b1-b4c8-71e08413e5fe', name: 'entitlement', value: ENTITLEMENT_TAGS[name] }] : [],
...
export const cloudAccountsInterceptions: InterceptionEntry[] = [
  ...
  { mock: ClientDataSourcesMock, url: '/v1/client/organizations/[^/]+/datasources' },
